...
const updateEnv = async () => {
    const pathToEnvPath = path.join(global.aql.appRoot, 'config', 'envPath');
    const file          = await fs.readFile(pathToEnvPath);
    let envPath         = file.toString();
    envPath             = path.join(global.aql.appRoot, envPath);
    ...
    global.aql.envFile = envFile[getEnv('AQUILA_ENV')];
};

/**
 * Get assign global.aql.envFile if envFile exists else stay null
 */
const getOrCreateEnvFile = async () => {
    try {
        global.aql.envPath = (await fs.readFile(path.join(global.aql.appRoot, 'config/envPath'))).toString();
    } catch (err) {
        await fs.writeFile(path.join(global.aql.appRoot, 'config/envPath'), 'config/env.json');
        global.aql.envPath = path.join(global.aql.appRoot, 'config/env.json');
    }

    try {
        let envFile;
        const envExample = JSON.parse(await fs.readFile(
            path.join(global.aql.appRoot, 'config/env.template.json'),
            {encoding: 'utf8'}
        ));
        if (fs.existsSync(path.resolve(global.aql.envPath))) {
            envFile = await fs.readFile(global.aql.envPath, {encoding: 'utf8'});
            if (envFile === '') {
                envFile = {};
            } else {
                try {
                    ...
                    const newPathTemp = `${global.aql.envPath}.temp`;
                    await fs.writeFile(newPathTemp, envFile);
                    console.error(`The content of ${global.aql.envPath} has been copied to ${newPathTemp}`);
                    envFile = {};
                }
            }
        ...
        await fs.writeFile(global.aql.envPath, JSON.stringify(envFile, null, 2));
        global.aql.envFile = envFile[getEnv('AQUILA_ENV')];
    } catch (err) {
        console.error(err);
        process.exit(1);
...
const controlNodeVersion = async () => {
    try {
        const packageJSON = JSON.parse(await fs.readFile(path.join(global.aql.appRoot, 'package.json'), {encoding: 'utf8'}));
...
const logVersion = async () => {
    ...
    if (global.aql.envFile.db) {
        console.log(`%s@@ Database : ${global.aql.envFile.db}%s`, '\x1b[32m', '\x1b[0m');
    }
    controlNodeVersion();
};

const startListening = async (server) => {
    if (global.aql.envFile && global.aql.envFile.ssl && global.aql.envFile.ssl.active) {
        const {key, cert} = global.aql.envFile.ssl;
        if (!key || !cert) {
            throw new Error('SSL Error - need a cert and a key file');
        }
        const keyPath  = path.resolve(global.aql.appRoot, key);
        const certPath = path.resolve(global.aql.appRoot, cert);
        try {
        ...
        try {
            spdy.createServer({
                key  : await fs.readFile(path.resolve(global.aql.appRoot, global.aql.envFile.ssl.key)),
                cert : await fs.readFile(path.resolve(global.aql.appRoot, global.aql.envFile.ssl.cert)),
                spdy : {
                    protocols : ['h2', 'http/1.1'],
                    plain     : false
                }
            }, server).listen(global.aql.port, (err) => {
                if (err) throw err;
                global.aql.isServerSecure = true;
                console.log(`%sserver listening on port ${global.aql.port} with HTTP/2%s`, '\x1b[32m', '\x1b[0m');
                server.emit('started');
            });
        } catch (error) {
            console.error(error);
        ...
        server.listen(global.aql.port, (err) => {
            if (err) throw err;
            console.log(`%sserver listening on port ${global.aql.port} with HTTP/1.1%s`, '\x1b[32m', '\x1b[0m');
            server.emit('started');
        });
    }
};
...
const getUploadDirectory = () => {
    if (global.aql.envConfig && global.aql.envConfig.environment) {
        const {photoPath} = global.aql.envConfig.environment;
        if (photoPath) {
            return photoPath;
        }
    }
