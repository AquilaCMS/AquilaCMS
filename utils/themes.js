    ...
    try {
        if (fs.existsSync(pathToInit)) {
            ...
            if (typeof initFileOfConfig.build === 'function') {
                ...
                returnValues          = await execCmd(`node -e "global.aquila.appRoot = '${appRoot}'; global.aquila.envConfig = '${globalEnvConfig}'; require('${slash(pathToInit)}').build()"`, slash(path.join(linkToTheme, '/')));
