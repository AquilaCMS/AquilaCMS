...
global.aql          = {};
global.aql.envPath      = null;
global.aql.envFile      = null;
global.aql.appRoot      = path.resolve(__dirname);
global.aql.port         = Number(process.env.PORT || 3010);
global.aql.defaultLang  = '';
global.aql.moduleExtend = {};
global.aql.translate    = require('./utils/translate');
...
const setEnvConfig = async () => {
    ...
    global.aql.envConfig = configuration.toObject();

    if ((await Configuration.countDocuments()) > 1) {
        console.error(`More than 1 configuration found ! _id '${global.aql.envConfig._id}' is use`);
    }
};
...
const initFrontFramework = async (themeName = null) => {
    ...
    let themeInfo;
    if (themeName === null) {
        themeName =  global.aql.envConfig.environment.currentTheme;
    }
        ...
        if (type === 'custom') {
            ...
            try {
                if (fs.existsSync(pathToInit)) {
                    ...
                    process.chdir(global.aql.appRoot);
...
const initServer = async () => {
    if (global.aql.envFile.db) {
        ...
        console.log(`%s@@ Admin : '/${global.aql.envConfig.environment?.adminPrefix}'%s`, '\x1b[32m', '\x1b[0m');

        // we check if we compile (default: true)
        const compile = (typeof global?.aql?.envFile?.devMode?.compile === 'undefined' || (typeof global?.aql?.envFile?.devMode?.compile !== 'undefined' && global.aql.envFile.devMode.compile === true));
        ...
        if (compile) {
            ...
            const themeFolder    = path.join(global.aql.appRoot, 'themes', currentTheme, '/');
