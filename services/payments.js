...
async function immediateCashPayment(req, method) {
    try {
        const modulePath     = path.join(global.aql.appRoot, `modules/${method.moduleFolderName}`);
...
async function deleteFailedPayment() {
    ...
    try {
        ...
        dateToDelete.setDate(dateToDelete.getDate() - (global.aql.envConfig.stockOrder.nbDaysToDeleteOlderFailedPayment || 30));
