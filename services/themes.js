...
const uploadTheme = async (originalname, filepath) => {
    if (path.extname(originalname) === '.zip') {
        ...
        const target_path      = path.join(global.aql.appRoot, 'themes');
...
const deleteTheme = async (themePath) => {
    ...
    const pathToTheme = path.join(global.aql.appRoot, complete_Path);
...
const getDemoDatasFilesName = async () => {
    const folder = path.join(global.aql.appRoot, `themes/${global.aql.envConfig.environment.currentTheme}/demoDatas`);
...
const copyDatas = async (themePath, override = true, configuration = null, fileNames = null) => {
    const themeDemoData = path.join(global.aql.appRoot, 'themes', themePath, 'demoDatas');
    const data          = [];
    let listOfFile      = [];
    ...
    const photoPath = path.join(global.aql.appRoot, require('../utils/server').getUploadDirectory());
...
const getCustomCss = async (cssName) => {
    ...
    for (const cssFolder of CSS_FOLDERS) {
        const fullPath = path.join(global.aql.appRoot, 'themes', themePath, cssFolder, `${cssName}.css`);
        try {
...
const getAllCssComponentName = async () => {
    try {
        ...
        const themePath = getThemePath();
        for (const cssFolder of CSS_FOLDERS) {
            const fullPath = path.join(global.aql.appRoot, 'themes', themePath, cssFolder);
            try {
                if (fs.existsSync(fullPath)) {
...
function getThemePath() {
    return global.aql.envConfig.environment.currentTheme;
}
...
async function languageManagement(theme = global.aql.envConfig.environment.currentTheme) {
    const pathToTheme = path.join(global.aql.appRoot, 'themes', theme, '/');
...
async function languageInitExec(theme = global.aql.envConfig.environment.currentTheme) {
    let returnValues;
    try {
        const pathToTheme        = path.join(global.aql.appRoot, 'themes', theme);
        ...
        if (isExist) {
            ...
            returnValues      = await packageManager.execCmd(`node -e "global.aql.appRoot = '${slash(global.aql.appRoot)}'; require('${slash(pathToLanguageInit)}').setLanguage('${tabLang}','${defaultLang}')"`, slash(path.join(pathToTheme, '/')));
...
const listTheme = async () => {
    const allTheme    = [];
    const pathToTheme = path.join(global.aql.appRoot, 'themes');
