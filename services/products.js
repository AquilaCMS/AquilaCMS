...
const newGetProductsByCategoryId = async (id, PostBody = {}, lang, isAdmin = false, user, reqRes = undefined) => {
    ...
    let products       = [];
    const productsList = menu.productsList;
    if (productsList && productsList.length > 0) {
        console.log('getProductsByCategoryId');
        const start = Date.now();
        for (let i = 0; i < productsList.length; i++) {
            products.push(await Products.findById(productsList[i].id).populate(PostBody.populate).lean({virtuals: true}));
        }
        // PostBody.filter._id = {$in: menu.productsList.map((item) => item.id.toString())};
        // products            = await Products.find(PostBody.filter).populate(PostBody.populate).lean({virtuals: true});
