...
const copyDatabase = async () => {
    ...
    try {
        ...
        try {
            await mongodump(global.aql.envFile.db);
            await mongorestore(global.aql.envFile.db);
        } catch (error) {
...
const anonymizeDatabase = async () => {
    ...
    const data     = mongoURI.parse(global.aql.envFile.db);
...
const dropDatabase = async () => {
    const mongodbUri     = mongoURI.parse(global.aql.envFile.db);
...
const dumpAnonymizedDatabase = async () => {
    try {
        ...
        const pathToArchive = path.join(global.aql.appRoot, pathUpload, 'temp', 'database_dump.gz');
