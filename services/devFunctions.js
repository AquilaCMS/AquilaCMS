...
const createModelData = async () => {
    ...
    const schemas        = [];
    const themeFolder    = path.join(global.aql.appRoot, 'themes', global.aql.envConfig.environment.currentTheme);
        ...
        try {
            ...
            for (const oneLine in require.cache) {
                ...
                for (const oneFolder of ['routes', 'services']) {
                    const pathToFolder = path.join(global.aql.appRoot, oneFolder);
