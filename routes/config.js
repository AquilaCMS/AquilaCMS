...
async function saveEnvConfig(req, res, next) {
    try {
        ...
        if (req.body?.environment?.needRestart || global.aql.envConfig.environment.needRestart) {
            setTimeout(() => {
        ...
        res.json({
            status : 'success',
            data   : {
                needRestart : req.body?.environment?.needRestart || global.aql.envConfig.environment.needRestart
            }
        });
    } catch (err) {
        return next(err);
