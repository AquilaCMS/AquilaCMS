...
const InitRoutes = (express, server) => {
    ...
    server.use(`/${global.aql.envConfig.environment.adminPrefix}`, adminFrontRouter);
    // loading backoffice
...
const loadAdminRoutes = (adminFrontRouter) => {
    const pathToAdminFile = path.join(global.aql.appRoot, 'routes', 'backoffice.js');
...
const loadDynamicRoutes = (app) => {
    ...
    const pathToRoutes = path.join(global.aql.appRoot, 'routes');
    const date         = Date.now();
...
const manageExceptionsRoutes = async (req, res, next) => {
    ...
    if (req.url.startsWith('/bo/') && !req.url.startsWith('/bo/api')) {
        if (path.basename(req.url).indexOf('.') > -1) {
            ...
            res.sendFile(path.join(global.aql.appRoot, url));
        } else {
            res.sendFile(path.join(global.aql.appRoot, 'bo/build/index.html'));
        }
    } else if (req.url.startsWith('/google')) {
        res.sendFile(path.join(global.aql.appRoot, req.url));
    } else if (req.url && req.url.startsWith('/images') && req.url.split('/').length === 6) {
        await require('../services/medias').getImageStream(req.url, res);
    } else if (
        global.aql.envConfig
        && req.url.length > global.aql.envConfig.environment.adminPrefix.length + 2
        && req.url.indexOf(`/${global.aql.envConfig.environment.adminPrefix}/`)  > -1
        && req.url.split('/')[req.url.split('/').length - 1].indexOf('.') > -1
    ) {
        let url = req.url.replace(global.aql.envConfig.environment.adminPrefix, 'backoffice').split('?')[0];
        if (fs.existsSync(path.join(global.aql.appRoot, url))) {
            res.sendFile(path.join(global.aql.appRoot, url));
        } else {
        ...
        if (!global.aql.installMode && !isAdmin) {
