...
export default function Account({ limit }) {
    ...
    return (
        <AccountLayout active="2">
            <NextSeoCustom
                noindex={true}
                title={`${environment?.siteName} - ${t('pages/account/orders:title')}`}
                description=""
            />
            
            <div className="container-tunnel-02">
                <h2 className="heading-2-steps">{t('pages/account/orders:titleNav')}</h2>
            </div>
                            ...
                            return (
                                <Fragment key={order._id}>
                                    <div className="w-commerce-commercecheckoutsummaryblockheader block-header">
                                        <h5 className="heading-6" style={{ width: '230px' }}>{t('pages/account/orders:order')} : #{order.number}</h5>
                                        ...
                                        <div className="lien_voir w-inline-block" style={{ cursor: 'pointer' }} onClick={() => onChangeViewOrders(index)}>
                                            <h6 className="heading-bouton-voir">{t('pages/account/orders:view')}</h6>
                                    ...
                                    <div className="section-detail-order" style={{ display: !viewOrders[index] ? 'none' : 'block' }}>
                                        <div className="container-tunnel-02">
                                            <h2 className="heading-5 center">{t('pages/account/orders:orderSummary')} : #{order.number}</h2>
                                        </div>
                                        <OrderDetails order={order} setOrders={setOrders} />
                                    </div>
                                </Fragment>
                            );
                        }) : <p>{t('pages/account/orders:noOrder')}</p>
                    }
                </div>
                {
                    pageCount > 1 && (
